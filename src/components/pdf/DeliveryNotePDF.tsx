...
export const DeliveryNotePDF = ({ order, items, company, partner }: DeliveryNoteProps) => (
    <Document>
        <Page size="A4" style={styles.page}>
            ...
            <View style={styles.header}>
                <View style={styles.headerLeft}>
                    <Text style={styles.logoPlaceholder}>{company?.name || 'Distribuidora S.A.'}</Text>
                ...
                <View style={styles.headerRight}>
                    ...
                    <View style={styles.textRow}>
                        <Text style={styles.label}>N° Pedido:</Text>
            ...
            <View style={styles.section}>
                ...
                <View style={styles.textRow}>
                    ...
                    <Text style={styles.value}>{partner?.city || ''} {partner?.address || ''}</Text>
                </View>
                <View style={styles.textRow}>
                    <Text style={styles.label}>Contacto:</Text>
                    <Text style={styles.value}>{partner?.phone || 'N/A'} - {partner?.email || 'N/A'}</Text>
                </View>
            </View>
                ...
                <View style={styles.table}>
                    ...
                    {items.map((item, i) => (
                        <View key={i} style={styles.tableRow}>
                            ...
                            <Text style={styles.col2}>{item.products?.name || 'Producto'}</Text>
                            <Text style={styles.col3}>{item.qty}</Text>
                            <Text style={styles.col4}>${Number(item.unit_price).toFixed(2)}</Text>
                            <Text style={styles.col5}>${Number(item.subtotal).toFixed(2)}</Text>
                        </View>
                    ))}
                </View>
            </View>
            ...
            <View style={styles.totalsContainer}>
                <View style={styles.totalsBox}>
                    <View style={styles.totalRow}>
                        ...
                        <Text>${Number(order.subtotal_usd || 0).toFixed(2)}</Text>
                    </View>
                    <View style={styles.totalRow}>
                        <Text style={styles.totalLabel}>TOTAL USD:</Text>
                        <Text style={styles.totalValueHigh}>${Number(order.total_usd || 0).toFixed(2)}</Text>
                    </View>
                    {order.exchange_rate && (
                        <View style={styles.totalRow}>
                            <Text style={styles.totalLabel}>TOTAL BS (BCV):</Text>
                            <Text>Bs. {Number(order.total_bs || 0).toFixed(2)}</Text>
                        </View>
                    )}
                </View>
            </View>
            ...
            <View style={styles.signatureBox}>
                ...
                <View>
                    ...
                    <Text style={{ textAlign: 'center', fontSize: 8, marginTop: 4, color: '#666' }}>Sello y Firma</Text>
                </View>
            </View>
