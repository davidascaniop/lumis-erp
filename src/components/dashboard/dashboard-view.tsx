...
import {
  Wallet, CheckCircle, ShoppingCart, Target, UserPlus, Package,
  BarChart3, DollarSign, ShieldCheck, Clock, AlertTriangle, ChevronRight, Bell,
} from 'lucide-react';
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 stagger">
        ...
        <Link href="/dashboard/cobranza?filter=overdue" className="block">
          <KpiCardWithSparkline
            ...
            iconBg={data.totalMora > 0 ? "bg-[rgba(255,45,85,0.10)]" : "bg-[rgba(0,229,204,0.10)]"}
            iconColor={data.totalMora > 0 ? "text-[#FF2D55]" : "text-[#00E5CC]"}
            glowColor={data.totalMora > 0 ? "rgba(255,45,85,0.12)" : "rgba(0,229,204,0.08)"}
        ...
        <div className="space-y-3">
          ...
          <div className="bg-surface-card border border-border rounded-2xl p-4 shadow-card hover-card-effect">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: "Nueva Venta", icon: ShoppingCart, href: "/dashboard/ventas/nueva", color: "#E040FB" },
                { label: "Registrar Cobro", icon: DollarSign, href: "/dashboard/cobranza", color: "#00E5CC" },
                { label: "Nuevo Cliente", icon: UserPlus, href: "/dashboard/clientes", color: "#4FC3F7" },
                { label: "Ver Reportes", icon: BarChart3, href: "/dashboard/reportes", color: "#FFB800" },
                ...
                >
                  ...
                  <span className="text-[10px] font-semibold text-text-2 group-hover:text-text-1 transition-colors leading-tight">
                    {label}
                  </span>
                </Link>
              ))}
          ...
          {data.pendingVerifications > 0 && (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <div className="w-8 h-8 rounded-lg bg-[rgba(224,64,251,0.15)] flex items-center justify-center flex-shrink-0">
                  <ShieldCheck className="w-4 h-4 text-[#E040FB]" />
                </div>
                <div>
                  <p className="text-sm font-semibold text-text-1">Verificar Cobros</p>
                  <p className="text-[11px] text-text-2">{data.pendingVerifications} pago(s) esperando</p>
                </div>
              </div>
              <Link
          ...
          {data.creditsDueToday.length > 0 && (
            <div className="bg-status-warn/5 border border-status-warn/15 rounded-2xl p-4 shadow-card hover-card-effect">
              ...
              <div className="space-y-2">
                {data.creditsDueToday.slice(0, 3).map((item: any) => (
                  ...
                  >
                    <p className="text-xs font-medium text-text-1 truncate max-w-[140px]">{item.client_name}</p>
                    <span className="font-primary text-xs text-status-warn">{formatCurrency(item.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-surface-card border border-border rounded-2xl p-5 shadow-card hover-card-effect">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-primary text-sm text-text-1">Top Clientes</h2>
            <Link
                  ...
                  <div key={client.partner_id}>
                    <div className="flex items-center justify-between mb-1.5">
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-[10px] text-text-3 w-4">#{i + 1}</span>
                        <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-[#E040FB]/20 to-[#7C4DFF]/20 flex items-center justify-center text-[9px] font-bold text-[#E040FB]">
                          {client.name?.[0]?.toUpperCase() ?? "?"}
                        </div>
                        <span className="text-xs font-medium text-text-1 truncate max-w-[120px]">{client.name}</span>
                      </div>
                      <span className="font-primary text-xs text-status-danger">{formatCurrency(client.total)}</span>
                    </div>
        ...
        <div className="bg-surface-card border border-border rounded-2xl p-5 shadow-card hover-card-effect">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-primary text-sm text-text-1">Más Vendidos</h2>
                  ...
                  <div key={p.product_id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-medium text-text-1 truncate max-w-[140px]">{p.name}</span>
                        <span className="font-primary text-[11px] text-ok ml-2 flex-shrink-0">{formatCurrency(p.revenue)}</span>
                      </div>
                      <div className="h-1 bg-surface-card/10 rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full"
                          style={{ width: `${pct}%`, background: "linear-gradient(90deg, var(--ok), var(--brand))" }}
                        />
                      </div>
                      <span className="text-[9px] text-text-3 mt-0.5 block">{p.qty} {p.unit}</span>
                    </div>
                  </div>
                );
              })}
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-surface-card border border-border rounded-2xl p-6 shadow-card hover-card-effect">
        <h2 className="font-primary text-base text-text-1 mb-2">Ventas por Mes</h2>
  ...
  return (
    <div className="bg-surface-card border border-border rounded-2xl p-6 shadow-card hover-card-effect">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="font-primary text-base text-text-1">Ventas por Mes</h2>
          <p className="text-xs text-text-2 mt-0.5">Facturación total en USD</p>
        </div>
        <div className="flex items-center gap-3 text-xs">
          ...
          {totalVentas > 0 && (
            <span className="font-primary text-text-1">{formatCurrency(totalVentas)}</span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-end gap-3 w-full h-[300px]">
        {filledData.map((d, i) => {
          const heightPct = d.total > 0 ? Math.max(12, (d.total / maxVal) * 100) : 4;
            ...
            <div key={d.month} className="flex-1 flex flex-col items-center gap-2 group">
              <span
                className={`text-[10px] font-primary transition-opacity ${
                  hasValue ? "text-brand opacity-100" : "text-text-3 opacity-0 group-hover:opacity-100"
                }`}
              >
                {hasValue ? formatCurrency(d.total) : "$0"}
              </span>
              <div
                className="w-full relative rounded-t-lg transition-all duration-200 group-hover:opacity-80 cursor-default"
                style={{
                  height: `${heightPct}%`,
                  background: hasValue
                    ? isLast
                    ...
                    : "var(--bg-card-hover)",
                  boxShadow: hasValue && isLast ? "var(--shadow-brand)" : "none",
                  borderRadius: "6px 6px 0 0",
                }}
              />
              <span
                className={`text-[10px] font-semibold ${
                  isLast ? "text-brand" : hasValue ? "text-text-2" : "text-text-3"
                }`}
              >
                {d.month}
              </span>
